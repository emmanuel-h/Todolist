...
- [Paper Background](paper-background.md) — both screens become one sheet of ruled loose-leaf paper: fixed ink-on-paper palette, light-only (no `values-night/`, no `DynamicColors`), 40dp gutter with punched holes and a double margin rule, per-row ruling replacing the `InsetItemDivider` decoration, card-free list rows, no background watermarks, paper-chip FAB, paper dialogs
